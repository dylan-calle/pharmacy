...
export default function DropDown({
  ...
  const filteredOptions = options.filter((option) =>
    option.name_raw_material.toLowerCase().includes(query.toLowerCase())
  );

  const handleOptionClick = (index, option) => {
    setSelectedOption(option);
    addSelectedMeasurement(option.measurement);
  ...
  const addSelectedMeasurement = (measurementToAdd) => {
    ...
    <div className="relative">
      <div className="flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md ">
        <input
          ...
          value={query} // Aqui porque no cambia cuando se elimina en add_raw_material
        ...
        >
          <img
            className="w-3 h-3"
            src="../../../src/assets/black-forward-icon.png"
            alt=""
          />
        </span>
      </div>
        ...
        <ul className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
          {filteredOptions.map((option) => (
            <li
              ...
              onMouseDown={() => handleOptionClick(index, option)}
            >
              <span
